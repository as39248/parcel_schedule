...
import json
from scheduler import Scheduler, RandomScheduler, GreedyScheduler
...
class SchedulingExperiment:
    ...
    def run(self, report: bool = False) -> Dict[str, Union[int, float]]:
        ...
        self._unscheduled = self.scheduler.schedule([Parcel(Parcel.parcel_id,
                                                            Parcel.volume,
                                                            Parcel.city1,
                                                            Parcel.city2)],
    ...
    def _compute_stats(self) -> None:
        ...
        self._stats = {
            'fleet': Fleet.num_trucks(Fleet()),
            'unused_trucks': Fleet.num_trucks(Fleet()) -
                             Fleet.num_nonempty_trucks(Fleet()),
            'avg_distance': Fleet.average_distance_travelled(Fleet(),
                                                             DistanceMap()),
            'avg_fullness': Fleet.average_fullness(Fleet()),
            'unused_space': Truck.max_capacity -
                            (Truck.fullness(Truck(Truck.truck_id,
                                                  Truck.max_capacity,
                                                  Truck.city)) / 100 *
                             Truck.max_capacity),
            'unscheduled': len(self.scheduler.
                               schedule([Parcel(Parcel.parcel_id,
                                                Parcel.volume,
                                                Parcel.city1,
                                                Parcel.city2)],
                                        [Truck(Truck.truck_id,
...
def read_parcels(parcel_file: str) -> List[Parcel]:
    ...
    with open(parcel_file, 'r') as file:
        for line in file:
            ...
            Parcel.parcel_id = pid
            Parcel.city1 = source
            Parcel.city2 = destination
            Parcel.volume = volume
            parcel.append(Parcel(pid, volume, source, destination))
    return parcel
...
def read_distance_map(distance_map_file: str) -> DistanceMap:
    ...
    with open(distance_map_file, 'r') as file:
        for line in file:
            ...
            DistanceMap().add_distance(c1, c2, distance1, distance2)
    return DistanceMap()
...
def read_trucks(truck_file: str, depot_location: str) -> Fleet:
    ...
    with open(truck_file, 'r') as file:
        for line in file:
            ...
            Truck.truck_id = tid
            Truck.max_capacity = capacity
            Truck.city = depot_location
    return Fleet()
